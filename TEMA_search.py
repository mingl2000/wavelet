...
def calculateSSA(symbol,ssa_df,df, window_size=13):
  ...
  if df is not None or len(df)<window_size:
    ...
    def print_ssa(symbol,X_ssa, V_ssa):    
      ...
      pass
    def add_tema_df(symbol,df,X_ssa, V_ssa,ssa_df, OBV_ssa, MF_ssa,theilslopes):
    ...
    #ssa_df=add_tema_df(symbol,df,temaC, temaV,ssa_df,OBV_ssa,MF_ssa,theilslopes)
    ssa_df=add_tema_df(symbol,df,temaC, temaV,ssa_df,None,None,theilslopes)
    return ssa_df
