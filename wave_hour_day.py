...
def GetYahooData(symbol, bars=500, interval='1d'):
  ...
  if interval.endswith('1m'):
    period='7d'
  elif  interval.endswith('m') or interval.endswith('d'):
    period='60d'
...
df_hour= GetYahooData(ticker,60,'30m')
df=df_hour
df_day= GetYahooData(ticker,60,'1d')
#plot_candle(df_day)
#plt.plot(df_hour, color='r')
#plt.plot(df_day, color='b')
#plt.show()
# import data
#df = pd.read_csv(filename, header=0, index_col=0, encoding='utf-8')
df.head()

#WT(index_list, wavefunc='db4', lv=3, m=1, n=3, plot=False):
wf_close=WT(df['Close'], wavefunc='db4',lv=7, m=1, n=7,plot=False)
wf_close_day=WT(df_day['Close'], wavefunc='db4',lv=3, m=1, n=3,plot=False)
wf_high=WT(df['High'], plot=False)
wf_low=WT(df['Low'], plot=False)
wf_vol=WT(df['Volume'], plot=False)

df["coeff_close"] = wf_close[0]
df_day["coeff_close"] = wf_close_day[0]
...
df_day_hl=pd.DataFrame().assign(High=df_day['High'], Low=df_day['Low'],Close=df_day['Close'],Open=df_day['Open'])
#plt.plot(df_day_hl)

df_wavelet_hour_CA=pd.DataFrame().assign(wavelet_hour=df['coeff_close'])
df_wavelet_day_CA=pd.DataFrame().assign(wavelet_day=df_day['coeff_close'])
plot_candle(df_day, df_wavelet_hour_CA,df_wavelet_day_CA)
#plt.plot(df_wavelet_CA)
#plt.show()
