...
def WT_WaveletPacket(df, wavefunc='db4',plot=False):
  ...
  new_wp = pywt.WaveletPacket(data=None, wavelet=wavefunc, mode='symmetric')
  threshold = np.std(data) / np.sqrt(2 * np.log2(len(data)))
  for node in wp.get_leaf_nodes(True):
    node.data = threshold_coeffs(node.data, threshold)
...
df.head()
WT_WaveletPacket(df,wavefunc='db4', plot=False)
