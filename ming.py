...
ticker="SPX"
filename=ticker +".csv"
df_HS300 = pd.read_csv(filename, header=0, index_col=0, encoding='utf-8')
df_HS300.head()
...
plt.plot(coeff[0], label = 'cA[%.0f]'%(len(coeff)-0-1))
plt.plot(test_C, label = 'real')
...
for i in range(len(results)):
    ...
    plt.plot(coeff[0], label = 'cA[%.0f]'%(len(coeff)-i-1))
    plt.legend(loc='upper right')
    plt.plot(results[i].fittedvalues, 'red', label='FittedValue')
    plt.legend(loc='upper right')
plt.savefig(("Wavelet.Arima." + ticker +"_8.png"))
plt.show()
# # ARMA_GBR/SVR
...
forward =30
window = 260 # the rolling window should be as large as possible. Better larger than 100, since the time series ARMA model can't get inverted matrics if the time series is too short 
length = 1
...
plt.legend(loc='best')
plt.savefig(("Wavelet.Arima." + ticker +"_9.png"))
plt.show()
# In[14]:
...
plt.legend(loc='best')
plt.savefig(("Wavelet.Arima." + ticker +"_10.png"))
plt.show()
# In[17]:
...
plt.plot(np.array(index_real)[:,0], 'blue', label='ActualValue') 
plt.legend(loc='best')
plt.savefig(("Wavelet.Arima." + ticker +".png"))
plt.show()
